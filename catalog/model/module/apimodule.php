    ...
    public function getOrders($data = array())
    {
        ...
        if (isset($data['filter'])) {
            ...
            if (isset($data['filter']['fio']) && $data['filter']['fio'] !== '' && $data['filter']['fio'] !== 'null' && $data['filter']['fio'] !== '0' && !ctype_digit($data['filter']['fio']) && intval($data['filter']['fio']) == 0) {
                ...
                $sql .= " ) ";
            } elseif (isset($data['filter']['fio']) && $data['filter']['fio'] !== '' && $data['filter']['fio'] !== 'null' && $data['filter']['fio'] !== '0' && intval($data['filter']['fio']) !== 0 && (strlen(intval($data['filter']['fio'])) < 6)) {
                $sql .= ' AND';
                $order_id = (int)$data['filter']['fio'];

                $sql .= " ( order_id = " . $order_id . "  ) ";
            } elseif (isset($data['filter']['fio']) && $data['filter']['fio'] !== '' && $data['filter']['fio'] !== 'null' && $data['filter']['fio'] !== '0' && intval($data['filter']['fio']) !== 0) {
                $sql .= ' AND';
                $str = str_replace(' ', '%', $data['filter']['fio']);

                $sql .= " ( telephone LIKE '%" . $str . "%'  ) ";
            }
